const PROCESSED_PREFIX = "catchcard:processed:";
const PROCESSED_TTL_SECONDS = 60 * 60 * 24 * 90;
const CLAIM_PREFIX = "claim:";

export function processedTweetKey(tweetId) {
  return `${PROCESSED_PREFIX}${String(tweetId)}`;
}
...
export async function tryClaimTweetForProcessing(kv, tweetId) {
  ...
  if (existing) {
    return {
      claimed: false,
      reason: isCompletedMarker(existing) ? "already_processed" : "in_progress",
    };
  }

  const claimId = `${CLAIM_PREFIX}${crypto.randomUUID()}`;
  await kv.put(key, claimId, { expirationTtl: PROCESSED_TTL_SECONDS });
